...
### 新マウス座標ベース表示制御（フルスクリーンゲーム対応・動的閾値計算）

#### 基本動作原理
- **マウスY座標 >= 1070（画面下端から10px以内）** AND **フルスクリーンゲーム非起動時** → TaskBarEx即座表示
- **マウスY座標が表示維持範囲内** → TaskBarEx表示継続
- **マウスY座標 < 非表示閾値（アプリバー上端より上）** OR **フルスクリーンゲーム起動時** → TaskBarEx即座非表示

#### 動的閾値計算システム（2024年4月実装）

**非表示閾値の動的計算:**
```cpp
int hideThreshold = m_screenHeight - m_taskbarHeight - m_appBarHeight;
// 例: 1080 - 48 - 86 = 946 (86px高さの2段レイアウト時)
```

**現在の2段レイアウト仕様:**
- **起動中アプリ行**: 40px高さ
- **ピン留めアプリ行**: 40px高さ  
- **アプリバー総高さ**: 86px（40+40+マージン6px）
- **アプリバー位置**: Y=946～1032（1080-48-86=946）
- **表示維持範囲**: Y=946～1070（124px幅）

#### 詳細動作仕様
- **画面高さ**: 1080ピクセル
- **表示閾値**: Y=1070（画面下端から10px以内で表示トリガー）
- **非表示閾値**: 動的計算（アプリバー上端で非表示トリガー）
...
- [x] **Phase 3.5**: 動的閾値計算システム（完了・2024年4月）
- [x] **Phase 3.6**: 固定2段レイアウト（完了・起動中+ピン留め、86px高さ）
- [ ] **Phase 4**: 多段表示システム（次期実装・アイテム数に応じた動的段数調整）
- [ ] **Phase 5**: 動的レイアウト調整（次期実装・2列表示対応）
- [ ] **Phase 6**: 高度なピン留めアプリ検出・表示（次期実装）
